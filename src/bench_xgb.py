...
def run_xgb_cv(
    ...
    # lazy import to avoid hard fail if xgboost not installed
    try:
        from xgboost import XGBClassifier
    except Exception as e:
        raise RuntimeError(
            "xgboost is not available in this environment. "
            "Install it (conda/pip) in your Sherlock env."
        ) from e
    ...
    print(f"[INFO] XGB params: n_estimators={n_estimators} max_depth={max_depth} lr={learning_rate} "
          f"subsample={subsample} colsample={colsample_bytree} lambda={reg_lambda} alpha={reg_alpha}")
    ...
    for fold, (tr_idx, te_idx) in enumerate(splits, start=1):
        ...
        # scale_pos_weight for imbalance (optional)
    ...
    print("\n===== CV Final Results (XGB mean pooling) =====")
...
def main():
    ...
    ap.add_argument("--out_dir", default="results_xgb")
