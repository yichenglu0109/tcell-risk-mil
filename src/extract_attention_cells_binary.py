...
@torch.no_grad()
def forward_with_attention(adata_p, ae, mil_models, device, assume_scaled=False, attn_reduce="median", pos_idx=1, prob_reduce="median"):
    ...
    for mil in mil_models:
        ...
        probs_pos.append(float(probs[0, int(pos_idx)].item()))
    ...
    p = np.asarray(probs_pos, dtype=float)
    if prob_reduce == "mean":
        prob_ens = float(np.mean(p))
    else:
        prob_ens = float(np.median(p))
    return prob_ens, w_ens
...
def main():
    ...
    ap.add_argument("--pos_idx", type=int, default=1, help="Positive class index in softmax(logits).")
    ap.add_argument("--prob_reduce", default="median", choices=["median", "mean"],
                    help="How to combine per-model positive probabilities.")
    ...
    for pid in pids:
        ...
        total_rows += len(df_p)
        written_patients += 1
        print(f"[INFO] pid={pid} prob_positive={prob_pos:.6f} n_cells={adata_p.n_obs} kept={len(df_p)}")
